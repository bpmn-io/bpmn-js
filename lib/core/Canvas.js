...
Canvas.prototype.viewbox = function(box) {
  ...
  if (!box) {
    ...
    eventBus.fire('canvas.viewbox.changed', { viewbox: this.viewbox() });
  }
...
Canvas.prototype.scroll = function(delta) {
  ...
  var scrolled = this._paper.node.createSVGMatrix().translate(delta.dx, delta.dy).multiply(node.getCTM());

  setCTM(node, scrolled);

  return { x: scrolled.e, y: scrolled.f };
};
