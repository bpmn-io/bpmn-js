...
var SvgTransformUtil = require('../util/SvgTransformUtil');
...
Canvas.prototype.getNativeDefaultLayer = function() {
  return this.getLayer(BASE_LAYER);
};

Canvas.prototype.getNativeLayer = function(name) {
...
Canvas.prototype._updateMarker = function(element, marker, add) {
  ...
  forEach([ container.gfx, container.secondaryGfx ], function(gfx) {
    if (gfx) {
      ...
      if (add) {
        domClasses(gfx).add(marker);
      } else {
        domClasses(gfx).remove(marker);
      }
    }
  });
...
Canvas.prototype.hasMarker = function(element, marker) {
  ...
  var gfx = this.getNativeGraphics(element);

  return domClasses(gfx).has(marker);
};
...
Canvas.prototype.getNativeGraphics = function(element, secondary) {
  return this._elementRegistry.getNativeGraphics(element, secondary);
};
...
Canvas.prototype.viewbox = function(box) {
  ...
  if (!box) {
    ...
    this._changeViewbox(function() {
      ...
      SvgTransformUtil.transform(viewport, -box.x, -box.y, 0, scale);
    });
  }
