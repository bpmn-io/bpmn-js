...
export default function BpmnTreeWalker(handler) {
  ...
  function handleCollaboration(collaboration, context) {
    ...
    //filter associations to be handled last
    let associations = []
    let notAssociations = []
    if(collaboration.artifacts) {
      associations = collaboration.artifacts.filter(function(artifact) {
        return is(artifact, 'bpmn:Association')
      });

      notAssociations = collaboration.artifacts.filter(function(artifact) {
        return !associations.includes(artifact);
      });
    } else {
      notAssociations = collaboration.artifacts;
    }

    handleArtifacts(notAssociations, context);

    deferred.push(function() {
      handleMessageFlows(collaboration.messageFlows, context);
    });

    handleArtifacts(associations, context);
  }
