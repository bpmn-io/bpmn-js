...
function notYetDrawn(semantic, refSemantic, property) {
  return new Error(_translate.t('element {element} referenced by {referenced_by}#{property} not yet drawn', {
                    element: elementToString(refSemantic),
                    referenced_by: elementToString(semantic),
                    property: property
                  }));
}
...
BpmnImporter.prototype.add = function(semantic, parentElement) {
  ...
  // CONNECTION
    ...
    throw new Error(translate.t('unknown di {element_di} for element {element_semantic}', {
...
BpmnImporter.prototype._attachBoundary = function(boundarySemantic, boundaryElement) {
  ...
  if (!hostSemantic) {
    throw new Error(translate.t('missing {boundary_semantic}#attachedToRef',
                            { boundary_semantic: elementToString(boundarySemantic) }));
  }
...
BpmnImporter.prototype._getEnd = function(semantic, side) {
  ...
  if (refSemantic) {
    ...
    throw new Error(translate.t('{element_semantic}#{side} Ref not specified',
                                  {element_semantic: elementToString(semantic), side: side}));
  }
};
