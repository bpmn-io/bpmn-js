...
var svgRemove = require('tiny-svg/lib/remove'),
...
function BendpointMove(injector, eventBus, canvas, dragging, graphicsFactory, rules, modeling) {
  ...
  function redrawConnection(data) {
    graphicsFactory.update('connection', data.connection, wrapSnapSvg(data.connectionGfx));
  }
  ...
  eventBus.on('bendpoint.move.start', function(e) {
    ...
    // add dragger gfx
    context.draggerGfx = BendpointUtil.addBendpoint(canvas.getNativeLayer('overlays'));
    domClasses(context.draggerGfx).add('djs-dragging');
