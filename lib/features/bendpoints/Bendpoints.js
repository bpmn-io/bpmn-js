...
function Bendpoints(eventBus, canvas, interactionEvents,
  ...
  function getBendpointsContainer(element, create) {
    ...
    if (!gfx && create) {
      ...
      domClasses(gfx).add('djs-bendpoints');
  ...
  eventBus.on('element.marker.update', function(event) {
    ...
    if (event.add) {
      domClasses(bendpointsGfx).add(event.marker);
    } else {
      domClasses(bendpointsGfx).remove(event.marker);
    }
  });
  ...
  eventBus.on('element.hover', function(event) {
    ...
    if (element.waypoints) {
      addHandles(element);
      interactionEvents.registerEvent(unwrapSnapSvg(event.gfx), 'mousemove', 'element.mousemove');
    }
  });

  eventBus.on('element.out', function(event) {
    interactionEvents.unregisterEvent(unwrapSnapSvg(event.gfx), 'mousemove', 'element.mousemove');
  });
