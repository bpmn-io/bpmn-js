...
function ConnectionSegmentMove(injector, eventBus, canvas, dragging, graphicsFactory, rules, modeling) {
  ...
  eventBus.on('connectionSegment.move.move', function(e) {
    ...
    if (segmentStartIndex < 2) {
      ...
      if (segmentStartIndex === 1) {
        ...
        if (sourceToSegmentOrientation === 'intersect') {
          ...
          // decrement segmentOffset also when source is on segment line
          // to provide correct hints
          if (Geometry.pointsOnLine(newSegmentStart, newSegmentEnd, LayoutUtil.getMid(connection.source))) {
            segmentOffset--;
          }
        }
      }
    ...
    if (segmentStartIndex >= 2) {
      ...
      var source = context.originalWaypoints[segmentStartIndex - 1],
          sourcePrev = context.originalWaypoints[segmentStartIndex - 2];

      // points on same line
      if (Geometry.pointsOnLine(newSegmentStart, newSegmentEnd, source)) {

        newSegmentStartIndex -= 2;

        if (axis === 'y' && source.y !== sourcePrev.y) {
          newSegmentStartIndex += 1;
        }

        if (axis === 'x' && source.x !== sourcePrev.x) {
          newSegmentStartIndex += 1;
        }
      }
    }
  ...
  eventBus.on('connectionSegment.move.end', function(e) {
    ...
    // TODO(@janstuemmel): filterRedundantWaypoints schould be refactored
    // within ConnectionSegmentMove.move method
