...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  if (bpmnElement.$instanceOf('bpmn:FlowNode')) {
    ...
    if (!bpmnElement.$instanceOf('bpmn:EndEvent') &&
      ...
      assign(actions, {
        'append.end-event': appendAction('bpmn:EndEvent', 'icon-01-none-end'),
        'append.gateway': appendAction('bpmn:ExclusiveGateway', 'icon-exclusive-gateway-variant-1'),
        'append.append-task': appendAction('bpmn:Task', 'icon-task'),
        'append.intermediate-event': appendAction('bpmn:IntermediateThrowEvent', 'icon-01-none-intermediate-throwing'),
        'connect': {
          group: 'connect',
