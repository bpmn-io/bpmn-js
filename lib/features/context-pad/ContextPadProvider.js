 ...
 * @param {import('../replace/BpmnReplace').default} bpmnReplace
 */
export default function ContextPadProvider(
    config, injector, eventBus,
    contextPad, modeling, elementFactory,
    connect, create, popupMenu,
    canvas, rules, translate, appendPreview, bpmnReplace) {
...
ContextPadProvider.prototype.getMultiElementContextPadEntries = function(elements) {
  ...
  function getReplaceMenuPosition(elements) {
  ...
  const differentTypes = !elements.every(element => element.type === elements[0].type);

  if (differentTypes) {

    var img = `<svg id="icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
  <path d="M18,28H14a2,2,0,0,1-2-2V18.41L4.59,11A2,2,0,0,1,4,9.59V6A2,2,0,0,1,6,4H26a2,2,0,0,1,2,2V9.59A2,2,0,0,1,27.41,11L20,18.41V26A2,2,0,0,1,18,28ZM6,6V9.59l8,8V26h4V17.59l8-8V6Z"/>
    ...
    assign(actions, {
      'filter': {
        group: 'edit',
        imageUrl: imgUrl,
        title: this._translate('Filter elements'),
        action: {
          click: (event, elements) => {
            const position = assign(getReplaceMenuPosition(elements), {
              cursor: { x: event.x, y: event.y }
            });

            this._popupMenu.open(elements, 'bpmn-filter', position, {
              title: this._translate('Filter elements'),
              width: 300,
              search: false
            });
  ...
  if (this._isReplaceAllowed(elements)) {
    assign(actions, {
      'replace': {
        group: 'edit',
        className: 'bpmn-icon-screw-wrench',
        title: this._translate('Change type'),
        action: {
          click: (event, elements) => {
            const position = assign(getReplaceMenuPosition(elements), {
              cursor: { x: event.x, y: event.y }
            ...
            // TODO: enable multi element popup menu
            this._popupMenu.open(elements, 'bpmn-replace', position, {
              title: this._translate('Change element'),
              width: 300,
              search: true
            });
          }
  ...
  if (this._isConnectAllowed(elements)) {
    // assign(actions, {
    //   'connect': {
    //     group: 'connect',
    //     className: 'bpmn-icon-connection-multi',
    //     title: this._translate('Connect to other element'),
    //     action: {
    //       click: () => {},
    //       dragstart: () => {},
    //     },
...
ContextPadProvider.prototype._isReplaceAllowed = function(elements) {
  return replaceOptions.reduce((result, option) => {
    const { target } = option;
    ...
    return elements.every(element => this._bpmnReplace.canReplace(element, target))
      && elements.some(element => isTargetDifferent(element, target));
  }, false);
};

ContextPadProvider.prototype._isConnectAllowed = function(elements) {
