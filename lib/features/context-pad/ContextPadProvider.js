...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  function appendAction(type, className, title, options) {
    ...
    if (typeof title !== 'string') {
      options = title;
      title = translate.t('Append {0}', {0: type.replace(/^bpmn\:/, '')});
    }
  ...
  if (isAny(businessObject, [ 'bpmn:Lane', 'bpmn:Participant' ])) {
    ...
    assign(actions, {
      'lane-insert-above': {
        ...
        title: translate.t('Add Lane above'),
        action: {
      ...
      if (element.height >= 120) {
        assign(actions, {
          'lane-divide-two': {
            ...
            title: translate.t('Divide into two Lanes'),
            action: {
      ...
      if (element.height >= 180) {
        assign(actions, {
          'lane-divide-three': {
            ...
            title: translate.t('Divide into three Lanes'),
            action: {
    ...
    assign(actions, {
      'lane-insert-below': {
        ...
        title: translate.t('Add Lane below'),
        action: {
          click: function(event, element) {
    ...
    if (isEventType(businessObject, 'bpmn:BoundaryEvent', 'bpmn:CompensateEventDefinition')) {
      ...
      assign(actions, {
        'append.compensation-activity':
            appendAction('bpmn:Task', 'bpmn-icon-task', translate.t('Append compensation activity'), {
  ...
  if (isAny(businessObject, [ 'bpmn:FlowNode', 'bpmn:InteractionNode' ]) ) {
    ...
    assign(actions, {
      ...
      'connect': {
        ...
        title: translate.t('Connect using ' +
  ...
  if (isAny(businessObject, [ 'bpmn:DataObjectReference', 'bpmn:DataStoreReference' ])) {
    assign(actions, {
      'connect': {
        group: 'connect',
        className: 'bpmn-icon-connection-multi',
        title: translate.t('Connect using DataInputAssociation'),
        action: {
          click: startConnect,
          dragstart: startConnect
        }
