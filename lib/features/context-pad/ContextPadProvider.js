...
export default function ContextPadProvider(
  ...
  eventBus.on([ 'create.hover', 'create.move' ], HIGH_PRIORITY, function(event) {
    var context = event.context,
        hints = context.hints || {},
        hover = event.hover;

    if (!hints.addToCollapsedSubProcess) {
      return;
    }

    if (hover && is(hover, 'bpmn:SubProcess') && !isExpanded(hover)) {
      var planeRoot = canvas.findRoot(getPlaneIdFromShape(hover));

      if (planeRoot) {
        event.hover = planeRoot;
        event.hoverGfx = canvas.getGraphics(planeRoot);
      }
    }
  });
}
...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  /**
   * Create an add-to-collapsed-sub-process action.
   *
   * @param {string} type
   * @param {string} className
   * @param {string} title
   ...
   * @return {ContextPadEntry}
   */
  function addToSubProcessAction(type, className, title, options) {

    function addStart(event, element) {

      var shape = elementFactory.createShape(assign({ type: type }, options));

      create.start(event, shape, {
        parent: element,
        hints: { addToCollapsedSubProcess: true }
      });
    }

    var add = autoPlace ? function(event, element) {
      var shape = elementFactory.createShape(assign({ type: type }, options));

      var planeRoot = canvas.findRoot(getPlaneIdFromShape(element));

      if (!planeRoot) {
        return;
      }

      modeling.createShape(shape, DEFAULT_POSITION, planeRoot);
    } : addStart;
  ...
  if (is(businessObject, 'bpmn:FlowNode')) {
    ...
    } else if (is(businessObject, 'bpmn:SubProcess') && !isExpanded(element)) {

      assign(actions, {
        'append.end-event': addToSubProcessAction(
          'bpmn:EndEvent',
          'bpmn-icon-end-event-none',
          translate('Add end event')
        ),
        'append.gateway': addToSubProcessAction(
          'bpmn:ExclusiveGateway',
          'bpmn-icon-gateway-none',
          translate('Add gateway')
        ),
        'append.append-task': addToSubProcessAction(
          'bpmn:Task',
          'bpmn-icon-task',
          translate('Add task')
        ),
        'append.intermediate-event': addToSubProcessAction(
          'bpmn:IntermediateThrowEvent',
          'bpmn-icon-intermediate-event-none',
          translate('Add intermediate/boundary event')
        )
      });
