...
export default function ContextPadProvider(
    ...
    canvas, rules, translate, appendPreview, autoConnect, connectionPreview) {
...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  var contextPad = this._contextPad,
      ...
      connectionPreview = this._connectionPreview;
  ...
  function startConnect(event, element) {
    ...
    connectionPreview.cleanUp();
  }

  function tryAutoConnect(event, element) {
    if (autoConnect.canConnect(element)) {
      autoConnect.connect(element);
    } else {
      connect.start(event, element);
    }
  ...
  function previewAutoConnect(event, element) {
    const context = {};

    const target = autoConnect.canConnect(element);

    if (target) {
      const canConnect = autoConnect.getConnectionType(element, target);
