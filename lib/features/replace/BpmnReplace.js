...
export default function BpmnReplace(
  ...
  function replaceElement(element, targetElement, hints) {
    ...
    var type = targetElement.type,
        ...
        targetTriggeredByEvent = type === 'bpmn:SubProcess' && !!targetElement.triggeredByEvent,
        shouldUpdateTriggeredByEvent = type === 'bpmn:SubProcess' &&
          targetTriggeredByEvent !== !!oldBusinessObject.triggeredByEvent;

    if (
      isSubProcess(oldBusinessObject) &&
      type === oldBusinessObject.$type
    ) {
      if (shouldToggleCollapsed(element, targetElement)) {
        if (shouldUpdateTriggeredByEvent) {
          return replaceSubProcess(element, targetElement, hints);
        }
  ...
  function replaceSubProcess(element, targetElement, hints) {
    var type = targetElement.type,
        oldBusinessObject = element.businessObject;
    ...
    if (is(oldBusinessObject, 'bpmn:Activity')) {
      ...
      if (isSubProcess(oldBusinessObject)) {
        ...
        if (targetElement && has(targetElement, 'isExpanded')) {
          newElement.isExpanded = targetElement.isExpanded;
        } else {

          // no toggling, so keep old state
