...
export default function EventBasedGatewayBehavior(eventBus, modeling) {
   ...
   *  source, and ensure only one connection exists between the gateway and
   *  each target.
   */
  this.preExecuted('shape.replace', function(event) {
    ...
    // remove duplicate connections from the event-based gateway to the same target
    newShape.outgoing.filter(isSequenceFlow).slice().forEach(function(sequenceFlow) {
      if (visitedTargets.indexOf(sequenceFlow.target) !== -1) {
        modeling.removeConnection(sequenceFlow);
      } else {
        visitedTargets.push(sequenceFlow.target);
      }
    });
  });
}
