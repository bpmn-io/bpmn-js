...
export default function LabelBehavior(
  ...
  // snap label bounds to text dimensions, anchoring position to the fixed edge
  this.preExecute('shape.resize', function(event) {
    ...
    var dimensions = getExternalLabelDimensions(shape, newBounds, textRenderer);

    var textWidth = Math.round(dimensions.width);
    var textHeight = Math.round(dimensions.height);
    ...
    context.newBounds = {
      width:  textWidth,
      height: textHeight,
      x: /w/.test(direction) ? right - textWidth : newBounds.x,
      y: /n/.test(direction) ? bottom - textHeight : newBounds.y
    };
  });
