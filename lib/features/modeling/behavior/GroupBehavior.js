...
export default function GroupBehavior(
  ...
  function removeCategoryValue(categoryValue, category, businessObject) {
    ...
    if (categoryValue &&
        !isReferencedCategoryValue(groups, categoryValue) &&
        !getCategorizedFlowElements(bpmnjs.getDefinitions(), categoryValue).length) {
      unlinkCategoryValue(categoryValue);
    }

    if (category &&
        !isReferencedCategory(groups, category) &&
        !category.get('categoryValue').length) {
