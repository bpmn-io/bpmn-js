...
/**
 * Return all flow elements that reference the given category value.
 *
 * @param {ModdleElement} definitions
 * @param {ModdleElement} categoryValue
 *
 * @return {ModdleElement[]}
 */
export function getCategorizedFlowElements(definitions, categoryValue) {
  return getFlowElements(definitions.get('rootElements')).filter(function(flowElement) {
    return flowElement.get('categoryValueRef').indexOf(categoryValue) !== -1;
  });
}


function getFlowElements(elements) {
  return elements.reduce(function(flowElements, element) {
    if (!element) {
      return flowElements;
    }
    ...
    if (is(element, 'bpmn:FlowElementsContainer')) {
      flowElements = flowElements.concat(getFlowElements(element.get('flowElements')));
    }
