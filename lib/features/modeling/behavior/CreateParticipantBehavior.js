...
export default function CreateParticipantBehavior(
    bpmnFactory, canvas, eventBus, moddleCopy, modeling) {
  ...
  eventBus.on('copyPaste.copyElement', function(context) {
    ...
    var businessObject = getBusinessObject(element),
        processRef = businessObject.get('processRef');

    if (processRef) {
      descriptor.referencedProcessRef = processRef;
    }
  });
  ...
  eventBus.on('copyPaste.pasteElement', LOW_PRIORITY, function(context) {
    ...
    var processRef = descriptor.referencedProcessRef;

    if (!processRef) {
      return;
    }

    processRef = moddleCopy.copyElement(
      processRef,
      bpmnFactory.create('bpmn:Process')
    );

    businessObject.set('processRef', processRef);
  });
}
