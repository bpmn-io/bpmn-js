...
/*   eventBus.on('drag.start', function(event) {
    ...
    if (!event.originalEvent.delegateTarget && !event.originalEvent.currentTarget.classList) {
      if (event.shape.type === 'bpmn:Participant') {
        forEach(event.shape.parent.children, function(s) {
          if (is(s, 'bpmn:Artifact')) {
            canvas.addMarker(s, 'selected');
          }
        });
      }
    }
  ...
  eventBus.on('shape.move.start', function(event) {
    ...
    if (shape.type === 'bpmn:Participant') {
      forEach(parent.children, function(s) {
        let curOverlap = 0;
        if (is(s, 'bpmn:Artifact')) {
          curOverlap = calculateOverlappingArea(shape, s);
          if (curOverlap > 0) {
            context.shapes.push(s);
          }
        }
      });
    }
...
/*   eventBus.on('selection.changed', function(event) {
    ...
    forEach(newSelection, function(e) {
      if (oldSelection.indexOf(e) === -1) {
        if (e.type === 'bpmn:Participant') {
          forEach(e.parent.children, function(s) {
            if (is(s, 'bpmn:Artifact')) {
              select(e);
              newSelection.push(s);
            }
...
/**
 * Updates Participants children / artifacts
 */
BpmnUpdater.prototype.updateParticipantsArtifacts = function(event) {
...
/*   // go through all possible parents to find the most overlapping one
  if (event.context.parent) {
    event.context.parent.children.forEach((child) => {
      var overlap = calculateOverlappingArea(shape, child);
      if (is(child, 'bpmn:Participant')) {
        if (overlap > curOverlap) {
          parent = child;
          curOverlap = overlap;
        }
      }
  ...
  if (is(shape, 'bpmn:Participant')) {
    ...
    const elementsToMove = [ ...enclosedArtifacts ];

    if (event.command === 'shape.move') {

      var delta = event.context.delta;
...
/* BpmnUpdater.prototype.updateParticipantsArtifacts2 = function(event) {
  const shape = event.context.shape;

  var parent,
      curOverlap = -1;

  if (event.context.newParent) {
    event.context.newParent.children.forEach((child) => {
      var overlap = calculateOverlappingArea(shape, child);
      if (is(child, 'bpmn:Participant')) {
        if (overlap > curOverlap) {
          parent = child;
  ...
  if (event.command === 'shape.created') {
