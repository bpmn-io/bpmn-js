...
export default function BpmnUpdater(
  ...
  eventBus.on('shape.changed', function(event) {
    if (event.element.type === 'label') {
      ...
      var dimensions = getExternalLabelDimensions(element, textRenderer);

      element.width = Math.round(dimensions.width);
      element.height = Math.round(dimensions.height);

      updateBounds({ context: { shape: element } });
    }
  });
