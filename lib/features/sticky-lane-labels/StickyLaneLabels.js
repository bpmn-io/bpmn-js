...
function collectOverlayEntry(overlayElement, canvasContainer, elementRegistry, previousViewportScale, overlayStateByContainer) {
  ...
  overlayElement.style.left = `${laneNestingOffset + PALETTE_OFFSET}px`;
  ...
  function addOverlays() {
    try {
      ...
      pools.forEach((pool) => {
        ...
        // if no lane or pool name is set, don't show overlays for them
        if (!name) {
          return;
        }
        ...
        // PALETTE_OFFSET accounts for the palette width + the gap between the palette and the overlay
