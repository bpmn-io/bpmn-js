...
ReplaceMenuProvider.prototype.getPopupMenuEntries = function(target) {
  ...
  if (is(businessObject, 'bpmn:FlowNode')) {
    ...
    return this._createEntries(target, withSubProcessStep(filteredReplaceOptions));
  }
...
/**
 * Fold sub-process replace options into a single step entry, keeping the
 * task replace menu shallow while still allowing incremental discovery.
 *
 * @param {ReplaceOption[]} options
 *
 * @return {ReplaceOption[]}
 */
function withSubProcessStep(options) {
  var subProcessOptions = filter(options, isSubProcessOption);

  if (subProcessOptions.length < 2) {
    return options;
  }

  var otherOptions = filter(options, function(option) {
    return !isSubProcessOption(option);
  });
  ...
  return [
    ...
    {
      label: 'Sub-process',
      actionName: 'sub-processes',
      className: 'bpmn-icon-subprocess-expanded',
      entries: subProcessOptions
    }
  ];
}
 ...
 * @return {boolean}
 */
function isSubProcessOption(option) {
  var type = option.target && option.target.type;
