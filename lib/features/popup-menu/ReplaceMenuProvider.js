...
ReplaceMenuProvider.prototype.getPopupMenuEntries = function(target) {
  ...
  if (is(businessObject, 'bpmn:AdHocSubProcess') && !isExpanded(target)) {
    filteredReplaceOptions = filter(replaceOptions.TASK, function(replaceOption) {
      var replaceTarget = replaceOption.target;

      if (replaceTarget.type === 'bpmn:SubProcess') {
        return replaceTarget.isExpanded === false;
      }

      if (replaceTarget.type === 'bpmn:AdHocSubProcess') {
        return replaceTarget.isExpanded === true;
      }
  ...
  if (is(businessObject, 'bpmn:SubProcess') && !isExpanded(target)) {
    var isCollapsedEventSubProcess = isEventSubProcess(businessObject);

    filteredReplaceOptions = filter(replaceOptions.TASK, function(replaceOption) {
      var replaceTarget = replaceOption.target;

      if (replaceTarget.type === 'bpmn:SubProcess') {
        var isTargetExpanded = replaceTarget.isExpanded === true;
        var isTargetEventSubProcess = !!replaceTarget.triggeredByEvent;

        if (isTargetExpanded) {
          return isTargetEventSubProcess === isCollapsedEventSubProcess;
        }
