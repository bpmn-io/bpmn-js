...
ReplaceMenuProvider.prototype.getPopupMenuEntries = function(target) {
  ...
  if (!rules.allowed('shape.replace', { element: target })) {
    return [];
  }
  ...
  if (is(businessObject, 'bpmn:Participant')) {
    ...
    entries = filter(replaceOptions.PARTICIPANT, function(entry) {
      return isExpanded(target) !== entry.target.isExpanded;
    });
 ...
 * @param {PopupMenuTarget} target
 *
 * @returns {PopupMenuHeaderEntries}
 */
ReplaceMenuProvider.prototype.getHeaderEntries = function(target) {

  var headerEntries = [];

  if (is(target, 'bpmn:Activity') && !isEventSubProcess(target)) {
    headerEntries = headerEntries.concat(this._getLoopEntries(target));
  }

  if (is(target, 'bpmn:DataObjectReference')) {
    headerEntries = headerEntries.concat(this._getDataObjectIsCollection(target));
  }

  if (is(target, 'bpmn:Participant')) {
    headerEntries = headerEntries.concat(this._getParticipantMultiplicity(target));
  }

  if (is(target, 'bpmn:SubProcess') &&
      !is(target, 'bpmn:Transaction') &&
      !isEventSubProcess(target)) {
    headerEntries.push(this._getAdHocEntry(target));
  }
