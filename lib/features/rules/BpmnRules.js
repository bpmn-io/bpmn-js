...
function canConnect(source, target, connection) {
  ...
  if (!is(connection, 'bpmn:DataAssociation')) {
    ...
    if (canConnectSequenceFlow(source, target, connection)) {
...
function canConnectSequenceFlow(source, target, connection) {

  if (
    isEventBasedTarget(target) &&
    target.incoming.length > 0 &&
    areOutgoingEventBasedGatewayConnections(target.incoming, connection) &&
...
function areOutgoingEventBasedGatewayConnections(connections, excludeConnection) {
  ...
  return connections.some(function(connection) {
    ...
    return isOutgoingEventBasedGatewayConnection(connection);
  });
}
