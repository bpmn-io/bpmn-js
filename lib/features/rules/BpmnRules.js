 ...
 */
export default function BpmnRules(eventBus, rules) {
...
BpmnRules.$inject = [ 'eventBus', 'rules' ];
...
BpmnRules.prototype.canConnectMessageFlow = function(source, target) {
  var allowed = this._rules.allowed('connection.create', {
    source: source,
    target: target
  });

  return allowed && allowed.type === 'bpmn:MessageFlow';
};

BpmnRules.prototype.canConnectSequenceFlow = function(source, target) {
  var allowed = this._rules.allowed('connection.create', {
    source: source,
    target: target
  });

  return allowed && allowed.type === 'bpmn:SequenceFlow';
};

BpmnRules.prototype.canConnectDataAssociation = function(source, target) {
  var allowed = this._rules.allowed('connection.create', {
    source: source,
    target: target
  });
  ...
  if (allowed &&
    allowed.type === 'bpmn:DataInputAssociation' ||
    allowed.type === 'bpmn:DataOutputAssociation') {
    return allowed;
  }
...
BpmnRules.prototype.canConnectAssociation = function(source, target) {
  var allowed = this._rules.allowed('connection.create', {
    source: source,
    target: target
  });

  return allowed && allowed.type === 'bpmn:Association';
};

BpmnRules.prototype.canMove = function(shapes, target) {
...
BpmnRules.prototype.canAttach = function(shape, target, source, position) {
  return this._rules.allowed('shape.attach', {
    shape: shape,
    target: target,
    position: position
  });
...
// TODO(philipp): correctly add rule so it can be intercepted by others
...
BpmnRules.prototype.canCreate = function(shape, target, source, position) {
  return this._rules.allowed('shape.create', {
    position: position,
    shape: shape,
    source: source,
    target: target
...
BpmnRules.prototype.canConnect = function(source, target) {
...
BpmnRules.prototype.canResize = function(shape, newBounds) {
  return this._rules.allowed('shape.resize', {
    shape: shape,
    newBounds: newBounds
  });
};

BpmnRules.prototype.canCopy = function(collection, element) {
  return this._rules.allowed('element.copy', {
