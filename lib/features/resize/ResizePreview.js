...
function ResizePreview(eventBus, elementRegistry, canvas, styles, previewSupport) {
  ...
  eventBus.on('resize.move', LOW_PRIORITY, function(event) {
    ...
    if (!frame) {
      frame = context.frame = previewSupport.addFrame(shape, canvas.getNativeDefaultLayer());
    ...
    if (context.canExecute) {
      domClasses(frame).remove(MARKER_RESIZE_NOT_OK);
    } else {
      domClasses(frame).add(MARKER_RESIZE_NOT_OK);
    }
