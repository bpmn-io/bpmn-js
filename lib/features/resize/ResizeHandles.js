...
ResizeHandles.prototype._createResizer = function(element, x, y, rotation, direction) {
  ...
  domClasses(group).add(CLS_RESIZER);
  domClasses(group).add(CLS_RESIZER + '-' + element.id);
  domClasses(group).add(CLS_RESIZER + '-' + direction);
  ...
  domClasses(visual).add(CLS_RESIZER + '-visual');
  ...
  domClasses(hit).add(CLS_RESIZER + '-hit');
...
ResizeHandles.prototype._getResizersParent = function() {
  return this._canvas.getNativeLayer('resizers');
};
 ...
 * @param {Number} angle
 */
function transform(element, x, y, angle) {
  ...
  svgTransform(element, [ translate, transform ]);
}
