...
function Resize(eventBus, elementRegistry, rules, modeling, canvas, selection, dragging) {
  ...
  function computeMinResizeBox(context) {
    ...
    // compute min dimensions based on children unless specified
    if (minDimensions === undefined) {
      minBounds = computeChildrenBBox(shape, context.childrenBoxPadding);
    }

    if (!minBounds) {
      // create min bounds from minimal dimensions
      minBounds = ResizeUtil.getMinResizeBounds(direction, shape, minDimensions || {
