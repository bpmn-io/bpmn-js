...
function Resize(eventBus, elementRegistry, rules, modeling, canvas, selection, dragging) {
  ...
  eventBus.on('resize.start', function(event) {
    ...
    // compute a minimum bounding box that
    // around the existing children
    if (children.length) {
      var gap = 20;
      boundaryBox = Elements.getBBox(children);

      // add a gap between the boundaryBox and the resizable container
      boundaryBox.width += gap * 2;
      boundaryBox.height += gap * 2;
      boundaryBox.x -= gap;
      boundaryBox.y -= gap;
    }
    ...
    event.context.children = children;
    event.context.boundaryBox = boundaryBox;
  });
  ...
  eventBus.on('resize.move', function(event) {
    ...
    var context = event.context,
        ...
        boundaryBox = context.boundaryBox,
        delta;
    ...
    if (boundaryBox) {
      context.newBounds = ResizeUtil.ensureMinBounds(context.newBounds, boundaryBox);
    }
