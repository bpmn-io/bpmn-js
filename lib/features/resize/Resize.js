 ...
 *    * childrenBoxPadding ({ left, top, bottom, right } || number) - Gap between the minimum bounding box and the container
 *
 *  f.ex:
 *
...
function Resize(eventBus, elementRegistry, rules, modeling, canvas, selection, dragging) {
  ...
  function computeMinBoundaryBox(context) {
    ...
    if (typeof context.childrenBoxPadding === 'object') {
      childrenBoxPadding = {
        left: context.childrenBoxPadding.left || 20,
        right: context.childrenBoxPadding.right || 20,
        top: context.childrenBoxPadding.top || 20,
        bottom: context.childrenBoxPadding.bottom || 20
      };
    } else {
      childrenBoxPadding = {
        left: context.childrenBoxPadding || 20,
        right: context.childrenBoxPadding || 20,
        top: context.childrenBoxPadding || 20,
        bottom: context.childrenBoxPadding || 20
      };
    }
    ...
    if (children.length) {
      ...
      minBoundaryBox.width += childrenBoxPadding.right * 2;
      minBoundaryBox.height += childrenBoxPadding.bottom * 2;
      minBoundaryBox.x -= childrenBoxPadding.left;
      minBoundaryBox.y -= childrenBoxPadding.top;
    } else {
