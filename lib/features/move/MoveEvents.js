 ...
 * @param {Rules} rules
 */
function MoveEvents(eventBus, dragSupport, modeling, rules) {
  ...
  function checkExecutable(dragContext) {
    dragContext.canExecute = rules.allowed('shapes.move', {
      shapes: dragContext.shapes,
      delta: dragContext.delta,
      newParent: dragContext.hoverElement
    });
  }
...
MoveEvents.$inject = [ 'eventBus', 'dragSupport', 'modeling', 'rules' ];
