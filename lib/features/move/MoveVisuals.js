 ...
 * @param {Styles} styles
 */
function MoveVisuals(events, selection, elementRegistry, canvas, snap, styles) {
  ...
  events.on('shape.move', function(event) {
    ...
    var dragContext = event.dragContext,
        ...
        hoverGfx = dragContext.hoverGfx;

    if (hoverGfx) {
      addDropMarkers(hoverGfx, dragContext.canExecute);
    }
