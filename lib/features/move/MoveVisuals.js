...
var MARKER_DRAGGING = 'djs-dragging',
    ...
    MARKER_NEW_TARGET = 'drop-new-target',
...
function MoveVisuals(eventBus, elementRegistry, canvas, styles) {
  ...
  function setMarker(element, marker) {
    ...
    [ MARKER_ATTACH, MARKER_OK, MARKER_NOT_OK, MARKER_NEW_TARGET ].forEach(function(m) {
  ...
  eventBus.on('shape.move.move', LOW_PRIORITY, function(event) {
    ...
    var context = event.context,
        ...
        primaryShapeParent = context.shape.parent;

    if (target) {
      if (canExecute === 'attach') {
        setMarker(target, MARKER_ATTACH);
      } else if (target !== primaryShapeParent && context.differentParents && context.canExecute) {
        setMarker(target, MARKER_NEW_TARGET);
      } else {
