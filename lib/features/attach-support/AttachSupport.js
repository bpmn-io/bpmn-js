...
function AttachSupport(eventBus, modeling, moveVisuals, rules) {
  ...
  // Prevent attachers from moving, when the space tool is performed.
  // Otherwise the attachers would be moved twice.
  eventBus.on('spaceTool.move', function(event) {
    ...
    forEach(movingAttachers, function(shape) {
      // Remove all attachers from the movingShapes, because they
