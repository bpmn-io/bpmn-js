...
var createMatrix = require('tiny-svg/lib/geometry').createMatrix,
    createTransform = require('tiny-svg/lib/geometry').createTransform;
...
function SpaceToolPreview(eventBus, elementRegistry, canvas, styles, previewSupport) {
  ...
  eventBus.on('spaceTool.selection.start', function(event) {
    var space = canvas.getNativeLayer('space'),
    ...
    domClasses(pathX).add('djs-crosshair');
    ...
    domClasses(pathY).add('djs-crosshair');
  ...
  eventBus.on('spaceTool.move', LOW_PRIORITY, function(event) {
    ...
    if (!context.dragGroup) {
      var spaceLayer = canvas.getNativeLayer('space');

      line = svgCreate('path');
      svgAttr(line, 'd', 'M0,0 L0,0');
      domClasses(line).add('djs-crosshair');
      ...
      svgAppend(canvas.getNativeDefaultLayer(), dragGroup);
    ...
    if (!context.frameGroup) {
      ...
      svgAppend(canvas.getNativeDefaultLayer(), frameGroup);
...
function translate(element, dx, dy) {
  var translationMatrix = createTransform(createMatrix(1, 0, 0, 1, dx, dy));

  svgTransform(element, translationMatrix);
}
