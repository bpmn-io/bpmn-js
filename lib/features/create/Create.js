...
var wrapSnapSvg = require('../../util/SnapSvgUtil').wrapSnapSvg;
...
function Create(eventBus, dragging, rules, modeling, canvas, styles, graphicsFactory) {
  ...
  function createVisual(shape) {
    ...
    svgAppend(canvas.getNativeDefaultLayer(), group);

    preview = svgCreate('g');
    domClasses(preview).add('djs-dragger');
    ...
    domClasses(visualGroup).add('djs-visual');
    ...
    graphicsFactory.drawShape(wrapSnapSvg(visual), shape);
