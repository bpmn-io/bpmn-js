...
export default function BpmnConnectSnapping(eventBus) {
  eventBus.on([
    ...
    if (hover && isAnyType(canExecute, [
      ...
      if (is(hover, 'bpmn:SequenceFlow')) {
        snapSequenceFlow(event, hover);
      }
...
function snapSequenceFlow(event, connection) {
  var position = getClosestPointOnLine(event, connection);
  snapToPosition(event, position);
}
