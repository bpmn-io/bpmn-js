 ...
 *
 * @param {Object} config
 * @param {Dimensions} config.defaultSize
 * @param {Dimensions} config.maxSize
 * @param {Number} config.boxPadding
 */
function LabelUtil(config) {

  config = _.extend({}, {
    defaultSize: DEFAULT_LABEL_SIZE,
    maxSize: MAX_LABEL_SIZE,
   ...
   * @param {String} options.fit how to fit into the bounding box. Any of { 'center-box', 'center-top' }
  ...
  function createLabel(parent, text, options) {
    ...
    var box = _.extend({}, defaultSize, options.box || {}),
        style = options.style || config.style,
        mode = options.fit || 'center-top';
    ...
    switch (mode) {
      case 'center-box':
        maxWidth = box.width - boxPadding * 2;
        break;

      default:
        maxWidth = MAX_LABEL_SIZE.width;
    }
